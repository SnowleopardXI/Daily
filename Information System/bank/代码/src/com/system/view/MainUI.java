...
public class MainUI extends JFrame {
	...
	public MainUI(Card card) {
		...
		addSpacing(leftPanel, gbcLeft);
		JButton details = new JButton("交易明细");
		details.setFont(new Font("微软雅黑", Font.BOLD, 16));
		details.setBackground(Color.gray);
		details.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				setVisible(true);
				new Transaction(card).setVisible(true);
			}
		});
		leftPanel.add(details, gbcLeft);

		// Right panel buttons
		addServiceButton("修改密码", new ChangePassword(card), gbcRight, rightPanel);
		addSpacing(rightPanel, gbcRight);

		JButton balance = new JButton("显示余额");
		balance.setFont(new Font("微软雅黑", Font.BOLD, 16));
		balance.setBackground(Color.gray);
		balance.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				setVisible(true);
				new ShowBalance(card).setVisible(true);
			}
		});
		rightPanel.add(balance, gbcRight);
