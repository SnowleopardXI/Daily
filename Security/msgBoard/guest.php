...
<div class="ds-post-main">
    <?php while ($row = mysql_fetch_array($lock_info)) { ?>
        <div class="ds-comment-body">
            <div id="ip">
                <span><?php echo preg_replace('/(\d+)\.(\d+)\.(\d+)\.(\d+)/is', "$1.$2.*.*", $row['ip_user_post']); ?></span><?php
...
<form action="./crud/addpost.php" method="post" class="elegant-aero">
    ...
    <label>
        <span>Message :</span>
        <textarea id="content" name="content" placeholder="请输入留言" required oninvalid="setCustomValidity('请输入留言')"
